...
#
# This example shows you how to run ChatGPT tasks in Union.
#
...
#
# `name` is for Union and it should be unique.
#
...
@union.task(
    ...
    secret_requests=[union.Secret(key="token", group="github-api")],
)
def get_github_latest_release(owner: str = "flyteorg", repo: str = "flyte") -> str:
    import requests

    token = union.current_context().secrets.get("github-api", "token")
...
def post_message_on_slack(message: str):
    ...
    token = union.current_context().secrets.get("slack-api", "token")
...
@union.dynamic(container_image=image)
...
@union.task(
    secret_requests=[
        union.Secret(key="bearer_token", group="tweet-api"),
        union.Secret(key="consumer_key", group="tweet-api"),
        union.Secret(key="consumer_secret", group="tweet-api"),
        union.Secret(key="access_token", group="tweet-api"),
        union.Secret(key="access_token_secret", group="tweet-api"),
    ],
)
def tweet(text: str):
    import tweepy
    ...
    TWEET_LENGTH = 280
    BEARER_TOKEN = union.current_context().secrets.get("tweet-api", "bearer_token")
    CONSUMER_KEY = union.current_context().secrets.get("tweet-api", "consumer_key")
    CONSUMER_SECRET = union.current_context().secrets.get("tweet-api", "consumer_secret")
    ACCESS_TOKEN = union.current_context().secrets.get("tweet-api", "access_token")
    ACCESS_TOKEN_SECRET = union.current_context().secrets.get("tweet-api", "access_token_secret")
