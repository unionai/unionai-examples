...
class ConvNormRelu(nn.Module):
    def __init__(
        ...
        self.act = (
            nn.LeakyReLU(negative_slope=0.2, inplace=False) if leaky else nn.ReLU(inplace=True)
        )
...
class PoseSequenceDiscriminator(nn.Module):
    def __init__(self, cfg):
        ...
        self.seq = nn.Sequential(
            ConvNormRelu(
                "1d", cfg.MODEL.DISCRIMINATOR.INPUT_CHANNELS, 256, downsample=True, leaky=leaky
            ),  # B, 256, 64
