...
Run (CPU only):
    flyte --config <your-config> run e1_dispatch_latency.py main --n-warm 500 --n-cold 10 --n-big 3
"""
...
big = flyte.Image.from_debian_base(name="skyrl-e1-big").with_pip_packages("vllm==0.11.0", "unionai-reuse")
...
big_env = flyte.TaskEnvironment(
    name="skyrl-e1-big", resources=flyte.Resources(cpu=1, memory="2Gi"), image=big
)
driver_env = flyte.TaskEnvironment(
    name="skyrl-e1-driver",
    resources=flyte.Resources(cpu=1, memory="1Gi"),
    image=small,
    depends_on=[pool_env, cold_env, big_env],
)
...
async def main(n_warm: int = 500, n_cold: int = 10, n_big: int = 3, batch: int = 100) -> dict:
    out: dict = {}
    ...
    # --- big image: sequential so each measures pull (first) then cached-node start (rest) ---
    bigs = []
    for i in range(n_big):
        bigs.append(await _timed(big_noop(i)))
    out["cold_big_image_sequential_s"] = [round(x, 1) for x in bigs]
