"""PyTorch LLaMA model.
Source: https://github.com/aws-neuron/neuronx-distributed/blob/main/examples/training/llama/modeling_llama_nxd.py
"""
    ...
    def forward(self, x):
        if self.pretraining_tp > 1:
            ...
            if self.config.selective_checkpoint_enabled:
                intermediate_states = checkpoint_method(
                    activation_mlp, gate_proj, up_proj
                )
            else:
...
class CoreAttention(nn.Module):
    ...
    def forward(self, query_states, key_states, value_states):
        ...
        causal_mask = torch.triu(
            torch.ones((1, 1, q_len, kv_seq_len), device="xla"), diagonal=1
        ).bool()
        ...
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=dtype).to(
            query_states.dtype
        )
...
class LlamaAttention(LlamaAttentionHF):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if self.config.qkv_linear:
            ...
            self.split_size = (
                self.num_heads * self.head_dim // get_tensor_model_parallel_size()
            )
        else:
        ...
        self.num_key_value_heads = neuronx_dist_utils.divide(
            config.num_key_value_heads * self.config.kv_shared_group_size,
            get_tensor_model_parallel_size(),
        )
        self.num_key_value_groups = self.num_heads // self.num_key_value_heads
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states, key_states, value_states, seq_len_dim_index = (
            self.permute_qkv_for_attn(
                query_states,
                key_states,
                value_states,
                bsz,
        ...
        query_states, key_states = apply_rotary_pos_emb(
            ...
            self.config.transpose_nki_inputs,
        )
        ...
        if self.config.sequence_parallel_enabled:
            ...
            attn_output = attn_output.reshape(
                q_len, bsz, self.hidden_size // get_tensor_model_parallel_size()
            )
        else:
            attn_output = attn_output.transpose(1, 2).contiguous()
            attn_output = attn_output.reshape(
                bsz, q_len, self.hidden_size // get_tensor_model_parallel_size()
            )
    ...
    def call_attn_impln(self, query_states, key_states, value_states):
        if self.use_ring_attention:
            attn_output = nki_ring_attn_func(
                query_states,
                key_states,
                value_states,
            ...
            attn_output = nki_flash_attn_func(
                ...
                key_states,
                value_states,
                transpose_nki_inputs=self.config.transpose_nki_inputs,
            )
        else:
            attn_output = self.core_attn(query_states, key_states, value_states)
        return attn_output

    def reshape_and_permute_states_for_fa(
        self, states, bsz, q_len, num_heads, head_dim, use_sequence_parallel
    ):
    ...
    def permute_qkv_for_attn(
        ...
        if (
            ...
            query_states = query_states.view(q_len, bsz, num_heads, head_dim).permute(
                1, 2, 0, 3
            )
            key_states = key_states.view(
                q_len, bsz, num_key_value_heads, head_dim
            ).permute(1, 2, 0, 3)
...
class LlamaDecoderLayer(LlamaDecoderLayerHF):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # self_attn returns (attn_output, attn_weights) — no past_key_value
    ...
    def forward(
        ...
        if position_ids is None:
            ...
            position_ids = torch.arange(
                start, start + seq_length, dtype=torch.long, device=device
            )
...
class LlamaRotaryEmbedding(LlamaRotaryEmbeddingHF):
    ...
    def forward(self, x, position_ids):
        ...
        # Core RoPE block
        inv_freq_expanded = (
            self.inv_freq[None, :, None].double().expand(position_ids.shape[0], -1, 1)
        )
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            freqs = (
                inv_freq_expanded.double() @ position_ids_expanded.double()
            ).transpose(1, 2)
...
def init_weights(module, device):
    ...
    if isinstance(module, LlamaRMSNorm):
        ...
        module, (ParallelEmbedding, RowParallelLinear, ColumnParallelLinear)
    ):
