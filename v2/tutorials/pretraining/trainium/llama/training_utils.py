"""Source: https://github.com/aws-neuron/neuronx-distributed/blob/main/examples/training/llama/training_utils.py"""
...
def get_param_groups_by_weight_decay(model):
    """Get param groups."""
    if (
        hasattr(model, "local_named_parameters")
        and hasattr(model, "partitioned")
        and model.partitioned
    ):
...
def create_instruction_based_dataset(
    data_dir, mini_batch_size, dp_size, dp_rank, seed, tokenizer=None, task=None
):
    if data_dir.startswith("s3://") or (
        os.path.isdir(data_dir) and os.path.exists(data_dir)
    ):
    ...
    train_data = train_dataset.shuffle().map(
        preprocess_train_dataset, remove_columns=train_dataset.column_names
    )
    ...
    test_data = test_dataset.map(
        preprocess_test_dataset, remove_columns=test_dataset.column_names
    )
...
def get_sin_cos_matrix(config):
    ...
    return emb.cos()[None, None, :, :].to(torch.float32), emb.sin()[
        None, None, :, :
    ].to(torch.float32)
...
def print_logs(
    ...
    logger.log(
        total_steps, loss, total_norm_cpu, current_lr, input_ids, throughput, start
    )
...
class Throughput:
    def __init__(
        ...
        self.seqs_per_iteration = (
            batch_size * world_size * grad_accum_usteps * logging_interval
        )
        self.moving_avg_window_size = math.ceil(
            moving_avg_window_size / logging_interval
        )
