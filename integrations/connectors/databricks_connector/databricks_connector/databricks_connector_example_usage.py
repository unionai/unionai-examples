...
@union.task(
    ...
    limits=union.Resources(mem="2000M"),
    cache_version="1",
)
def hello_spark(partitions: int) -> float:
    print(f"Starting Spark with {partitions} partitions.")
    n = 100000 * partitions
    sess = union.current_context().spark_session
...
# Additionally, we define a standard Union task that won't be executed on the Spark cluster.
@union.task(cache_version="1")
