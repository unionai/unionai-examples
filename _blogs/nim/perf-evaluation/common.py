...
class MetricsCollector:
    ...
    def report_final(self):
        ...
        if head_latency_size:
            head_latencies = [
                j for i in self.response_head_latency_bucket.values() for j in i
            ]

            print(
                f"Average Response Head Latency: {sum(head_latencies) / head_latency_size}"
            )
            print(f"Median Response Head Latency: {np.percentile(head_latencies, 50)}")
            print(f"95% Response Head Latency: {np.percentile(head_latencies, 95)}")
            print(f"99% Response Head Latency: {np.percentile(head_latencies, 99)}")
        ...
        if latency_size:
            ...
            print(f"Average Response Latency: {sum(latencies) / latency_size}")
            print(f"Median Response Latency: {np.percentile(latencies, 50)}")
            print(f"95% Response Latency: {np.percentile(latencies, 95)}")
            print(f"99% Response Latency: {np.percentile(latencies, 99)}")
